...
// Gaussian about params.average_p*. The cylinder's height axis is
...
// where average_p* points). Advances gen's state, so a caller generating a
// whole beam must share one generator across calls to get a single
