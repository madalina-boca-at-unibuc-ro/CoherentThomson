...
create_detector(const ConfigMap& config) {
  ...
  // The detector's own normal direction, given in the simulation frame (the laser always propagates
  // along Oz).
  ...
  if (type == "rectangular") {
    ...
    return std::make_unique<SphericalDetector>(
        ...
        theta_min_val, theta_max_val, phi_min_val, phi_max_val, dir_x, dir_y, dir_z);
    ...
    return std::make_unique<CircularDetector>(
        ...
        detector_distance_val, R_min_val, R_max_val, dir_x, dir_y, dir_z);
  } else {
