...
void export_incident_field_fourier(const Laser::LaserField& laser, const Detector::Detector_2D& detector,
  ...
  if (detector_type != "RectangularDetector" && detector_type != "CircularDetector") {
    throw std::runtime_error("export_incident_field_fourier: detector_type '" + detector_type +
                             ...
                             "CircularDetector are supported.");
  }
  ...
  for (size_t i = 0; i < N1; ++i) {
    for (size_t j = 0; j < N2; ++j) {
      ...
      // Canonical-frame position (t=ct0/c, x_loc, y_loc, z_loc=0), built directly from the laser's
      // own transverse basis vectors -- bypassing the detector's actual to_lab_frame rotation/
      // distance entirely, so this screen sits at the laser's own waist regardless of the real
      // detector geometry.
