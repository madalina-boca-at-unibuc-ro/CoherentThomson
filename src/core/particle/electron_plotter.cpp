...
void plot_particle_trajectory(const Electron& electron, const std::string& filepath) {
  ...
  file << "# individual electron trajectory\n";
  file << "# store_trajectory value " + IoUtils::convert_bool_to_string(electron.get_store_trajectory()) << "\n";
  file << "tau x0 x1 x2 x3 p0 p1 p2 p3\n";
  ...
  std::cout << "Successfully exported electron trajectory to " << filepath << "\n";
}
